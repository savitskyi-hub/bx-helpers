...
(function() {
	 ...
	 */
	SavitskyiHub.BxHelpers.Helpers.Content.FixedBlockOnScroll = {
		/**
		 ...
		 * @param string scrollBlock - селектор блока который будет фиксироватся при скроллинге (название класса должно включать - "block-scroller-y")
		...
		/**
		 * Метод инициализирует работу фиксированых блоков которые будут фиксироваться при достижении условия, что определены в свойстве "obListFixedBlock"
		 *
		...
		init : function(obInitItem) {
			...
			if (obInitItem !== undefined && BX.type.isPlainObject(obInitItem)) {
				obList = obInitItem;
			...
			if (BX.type.isNotEmptyObject(obList)) {
				for (p in obList) {
					...
					arFunction[p] = function(key) {
						...
						let scrollingTagNode = document.querySelector(arFuncProp.scrollingTag),
							scrollBlockNode = document.querySelector(arFuncProp.scrollBlock),
						...
						let namespace = BX.Local.Helpers.Content.FixedBlockOnScroll,
						...
						scrollTop = (!arFuncProp.scrollingTag? pageYOffset : scrollingTagNode.scrollTop);
